# Building a Flexible Extension System for the IAC Prototype
...
```mermaid
graph TD
    A["📦 Developer Creates Extension"] -->|Packages code + manifest.json| B["Create ZIP Archive"]
    B -->|Computes SHA256 hash| C["Generate Hash"]
    C -->|Registers in extensions/registry.json| D["Publish to Registry"]
    
    D -->|User clicks Install| E["User Initiates Install"]
    E -->|POST /api/extensions/install| F["Backend Receives Request"]
    F -->|Downloads from zipFile URL| G["Download ZIP"]
    G -->|Compares SHA256| H["Verify Hash Integrity"]
    
    H -->|If valid, extract| I["Extract ZIP to ~/.iac/extensions/id/"]
    I -->|Generate resolved settings| J["Generate host_config.json"]
    J -->|Write to extension dir| K["Write Configuration"]
    K -->|Return install path| L["Installation Complete"]
    
    L -->|User sees in sidebar| M["Display in UI"]
    M -->|User configures settings| N["Update Settings"]
    N -->|Save to localStorage| O["Settings Persisted"]
    
    O -->|User launches extension| P["Start Extension Process"]
    P -->|Extension runs on assigned port| Q["Extension Running"]
    Q -->|Extension executes tests| R["Extension Executes"]
    R -->|Posts results via postMessage| S["Collect Results"]
    S -->|Store in localStorage| T["Results Saved"]
    T -->|Display in Assurance Results| U["View Results"]
    
    U -->|User clicks Uninstall| V["User Initiates Uninstall"]
    V -->|DELETE /api/extensions/uninstall| W["Backend Receives Request"]
    W -->|Delete ~/.iac/extensions/id/| X["Remove Extension Directory"]
    X -->|Update localStorage| Y["Remove from Sidebar"]
    Y -->|Cleanup Complete| Z["Uninstallation Complete"]
