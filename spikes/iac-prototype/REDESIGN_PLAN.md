...
| 8 | Cross-referencing + AgenticGoat Internal target | — |
...
| 13 | `iac-copilot-api` test suite | — |
| 14 | Dead-code cleanup | — |
| 15 | Project concept — real design needed (MVP1 curated / MVP2 user-authored) | — |
...
8. **Cross-referencing + AgenticGoat Internal target.** *Added 2026-08-26, made concrete.* Two parts:
   - **AgenticGoat Internal** — a lightweight, intentionally-vulnerable target app that actually runs, for demos, replacing today's metadata-only `agenticGoatSeed.ts` placeholder. Ships with the product the same way `host_skills/` does (repo-committed, e.g. `iac-copilot-api/agentic_goat/`, always available, no install step) — a small local service (own port) exposing a handful of intentionally-vulnerable endpoints spanning a few OWASP LLM Top 10 categories, started manually alongside the rest of the stack like everything else in this repo (no auto-orchestration exists here). Registered as a `target`-type Registry entry whose `targetUrl` points at the real running local service, tagged so the UI can show it's built-in.
   - **Unified target selection.** Testing projects' Target tab (currently a `ModelProviderContext` stand-in per item 1) gets rewired to `useRegistry()`, listing every `target`-type Registry entry — AgenticGoat Internal (always there) alongside anything a user registers externally. No special-casing needed for "built-in vs. external": both are just Registry entries, selected through the same picker, which is exactly what the Registry-as-source-of-truth decision above was for. **Open question, not yet answered:** whether the 10 canned-Project testers currently have *any* concept of a configurable target at all — their internals were never inspected this session (they were wrapped as-is, not audited). Wiring target selection through to each tester's actual execution logic needs that investigated first; it may not be a simple prop-pass.
...
13. **`iac-copilot-api` test suite.** No test suite exists anywhere in the backend today — everything built this session (Skill Package install/discovery, the skill runner) was verified via ad-hoc scripts, not anything repeatable that guards against future regressions. Worth standing up `pytest`/`pytest-asyncio` and porting the ad-hoc verification (zip-slip cases, containment-check cases, mocked tool-dispatch flow) into real tests.
...
15. **Project concept — real design needed.** *Added 2026-08-26.* What shipped in item 1 is a thin wrapper (Overview/Target/Run tabs around one existing tester) — real, working, but not the actual concept. The intended shape, per direct product-owner input: a Project is a group of **conversations and tool executions**, with **instructions** and **resources** scoping what it's for — closer to a persistent workspace than a one-shot test run. This needs its own design pass, not an incremental tweak to `Project.tsx`.
    - **Curated vs. user/team-authored splits the same way Skills already did.** Canned Projects = curated by the HACTU8 core team, shipped with the product (the same "always there, no install step" pattern as `host_skills/` and the OWASP-curated Skills path). User/team Projects = created by users or teams themselves (the same role the in-app Skills Creator plays). This consistency is a good sign, not a coincidence — worth leaning into rather than inventing a fourth pattern.
    - **Proposed staging, and it lines up with how Skills was actually built**: **MVP Milestone 1 = the curated version** (consume/run canned Projects — simpler, no ownership/team/auth concerns), **MVP Milestone 2 = user/team authoring**. Skills followed the same order for unrelated reasons (install/discovery shipped before the Creator rework) and it worked well — building the read/consumption side first, then the authoring side once its target shape is proven, is a validated pattern here now, not just a guess.
    - **Open design question, not yet answered:** is a Project its own new bundle format, or built on the same primitive Skill Packages already established (SKILL.md-style instructions + a `resources/`-equivalent directory)? This isn't a new idea — "Console, Projects... simple for now: an INSTRUCTION.md naming which installed skill to run" was floated earlier in the Skills Format discussion above and never resolved. Given how much infrastructure now exists for that shape (directory discovery, frontmatter parsing, containment-checked execution), reusing it for Projects instead of inventing a parallel format is the leading candidate — worth a real plan-mode pass before building either milestone.
    - Depends on item 8's open question too: whether the existing 10 canned testers even have a configurable-target concept to build "tool executions" on top of.
